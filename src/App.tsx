      ...
      <main className="flex-1 relative z-10 overflow-y-auto custom-scrollbar flex flex-col">
            ...
            <div className="flex items-center gap-6">
               {state.droneDetected ? (
                 ...
                 <div className="flex items-center gap-2 px-3 py-1 bg-white/5 border border-white/10 rounded-none text-[8px] tech-mono text-white/20">
                    <Scan size={10} /> SEARCHING_FOR_DOCKS...
                 </div>
               )}
         ...
         <footer className="h-8 border-t border-white/5 bg-black/60 flex items-center justify-between px-8 text-[7px] tech-mono uppercase tracking-[0.3em] text-white/10">
            <div className="flex gap-6">
               ...
               <span className={state.droneDetected ? 'text-red-500' : 'text-nes-blue/40'}>
                 SKY: {state.droneDetected ? 'TARGET_DETECTED' : 'CLEAR'}
               </span>
            </div>
            <div className="flex gap-6 items-center">
               <a 
                 href="https://github.com/ghostintheprompt/duckhunter-drone-defense-laboratory/releases" 
                 target="_blank" 
                 rel="noopener noreferrer"
                 className="hover:text-white transition-colors border-r border-white/10 pr-6"
               >
                  CHECK_FOR_UPDATES
               </a>
               <span className="text-zapper-orange animate-pulse">
                 {state.droneDetected ? `SIGNAL: -${state.signalStrength.toFixed(0)}dBm` : 'SCANNING_OS'}
               </span>
               <span>DUCKHUNT_OS_V1.1</span>
            </div>
         </footer>
      </main>
    </div>
